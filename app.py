...
load_dotenv()
API_ID = os.getenv("TG_API_ID", "31416036")
API_HASH = os.getenv("TG_API_HASH", "1d5c21a3ce62a0ff85c24ea4d24e33c5")
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            if row:
                ...
                cursor.execute("INSERT OR IGNORE INTO device_usage (device_id, search_count, last_search_time) VALUES (?, 0, 0)", (device_id,))
                conn.commit()
                return {"search_count": 0, "last_search_time": 0.0, "is_blocked": False}
    except Exception as e:
        logger.error(f"DB get_device_info error: {e}")
        return {"search_count": 0, "last_search_time": 0.0, "is_blocked": False}
...
def clean_html_report(raw_text: str) -> str:
    ...
    try:
        ...
        logger.warning(f"HTML cleaner fallback: {e}")
...
def safe_translate(text: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Translation bypassed: {e}")
        return text
    ...
    for attempt in range(3):
        try:
            if not client:
                ...
                session = StringSession(TG_SESSION_STRING.strip())
                client = TelegramClient(session, api_id_int, API_HASH)

            if not client.is_connected():
                logger.info(f"Connecting Telegram client (attempt {attempt + 1}/3)...")
...
# Keep-Alive Background Worker (pings every 3 minutes)
async def keep_alive_worker():
    ...
    while True:
        try:
            ...
            if render_url:
                urllib.request.urlopen(f"{render_url}/api/status", timeout=8)
        except Exception:
            pass
...
async def lifespan(app: FastAPI):
    logger.info("Initializing Bulletproof Core Engine...")
    await ensure_connected()
...
async def reset_cooldown(req: ResetRequest):
    ...
    return {
        "status": "success", 
        "message": "Reset successful for device."
    }
...
async def fetch_all_paginated_pages(client: TelegramClient, target_bot: str, main_msg) -> List[str]:
    ...
    while current_msg and page_count <= max_pages:
        ...
        try:
            ...
            logger.warning(f"Pagination click stopped: {e}")
            break
            ...
            if downloaded_bytes:
                try:
                    ...
                    if len(cleaned_dump.strip()) > 20:
                        logger.info(f"Successfully downloaded full file dump ({len(cleaned_dump)} chars).")
                        return cleaned_dump.strip()
                except Exception:
                    pass
    except Exception as e:
        logger.warning(f"Download extraction: {e}")
...
async def search_number(req: SearchRequest):
    ...
    if dev["search_count"] >= MAX_SEARCH_LIMIT or dev["is_blocked"]:
        return JSONResponse(
            ...
            content={
                "error": "limit_reached",
                "message": f"Device Limit: {MAX_SEARCH_LIMIT}/{MAX_SEARCH_LIMIT} searches used. Access is locked.",
                "searches_left": 0
            }
        )
    ...
    # 3. Ensure Telegram Connection with Retry
    is_connected = await ensure_connected()
    if not is_connected:
        return JSONResponse(
            status_code=503,
            content={
                "error": "system_busy",
                "message": "System is connecting to network. Please retry in 5 seconds."
            }
        )
        ...
        try:
            ...
            async with client.conversation(BOT_USERNAME, timeout=45) as conv:
                ...
                try:
                    ...
                    raise HTTPException(status_code=504, detail="Database lookup timed out. Please retry.")
            ...
            # Translate & format collected messages safely
            translated_messages = []
            ...
            divider = "\n\n" + "═" * 42 + "\n\n"
            final_text = divider.join(translated_messages) if translated_messages else "No data found for this query."
            ...
            return JSONResponse(
                status_code=429,
                content={"error": "flood_wait", "message": f"Rate limit reached. Please wait {e.seconds} seconds."}
            )
            ...
            return JSONResponse(
                status_code=500,
                content={"error": "query_failed", "message": "Database query could not complete. Please retry in a moment."}
            )
